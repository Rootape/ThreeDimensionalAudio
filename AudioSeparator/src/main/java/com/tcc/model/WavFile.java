...
public class WavFile extends ByteUtils{
	...
	public void changeVolume(double m) {
		for(int i = 0; i < this.data.length; i++) {
			this.data[i] = (int)(this.data[i] * m);
		}
	}
	
	public void sumOfFiles(WavFile other, double m) {
		int[] data2 = other.getData();
		for(int i = 0; i < data2.length; i++) {
			this.data[i] = this.data[i] + (int)(data2[i] * m);
		}
	}
	
	public void printWav(int c) {
		
		byte[] infos = this.getOriginal();
		
		for(int i = 0; i < c; i ++) {
			System.out.print("| ");
			for(int j = 12*i; j < 12*(i+1); j++) {
				System.out.print(Util.byteToHex(infos[j]) + " ");
			}
			if(!(i%2==0)) {
				System.out.print("|\n");
			}
